(() => {
  ...
  function i2osp(num, byteLength) {
    ...
    const unpadded = new Uint8Array(sjcl_default.codec.bytes.fromBits(num.toBits(void 0)));
  ...
  var BlindRSA = class _BlindRSA {
    ...
    async finalize(publicKey, msg, blindSig, inv) {
      ...
      const algorithm = { name: _BlindRSA.NAME, saltLength: this.params.saltLength };
    ...
    static generateKey(algorithm) {
      return crypto.subtle.generateKey({ ...algorithm, name: _BlindRSA.NAME }, true, [
        "sign",
        "verify"
      ]);
    }
    generateKey(algorithm) {
      return _BlindRSA.generateKey({ ...algorithm, hash: this.params.hash });
    }
