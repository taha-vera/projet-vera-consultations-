...
# --------------------------------------------------------------------------
# Endpoints de TEST pour la signature aveugle (Porte 7 durcie) -- isoles
# du systeme de production actuel, ne modifient aucun registre existant.
# A utiliser uniquement pour valider le nouveau systeme en parallele,
# jamais encore branche sur le vrai flux de vote.
# --------------------------------------------------------------------------

class TestGenererSigneRequete(BaseModel):
    departement: str


@app.post("/api/test/generer_token_signe")
def test_generer_token_signe(payload: TestGenererSigneRequete, session_vera: Optional[str] = Cookie(None)):
    exiger_session(session_vera)
    if not SIGNATURE_AVEUGLE_DISPONIBLE:
        raise HTTPException(
            status_code=503,
            detail="Module de signature aveugle non disponible sur ce serveur (vera_blind_sig non compile pour cette plateforme).",
        )
    token_signe = gestionnaire_signature.generer_token_signe(payload.departement)
    return {"token_signe": token_signe}


class TestVerifierSigneRequete(BaseModel):
    token_signe: dict


@app.post("/api/test/verifier_token_signe")
def test_verifier_token_signe(payload: TestVerifierSigneRequete):
    if not SIGNATURE_AVEUGLE_DISPONIBLE:
        raise HTTPException(status_code=503, detail="Module de signature aveugle non disponible.")
    try:
        departement = gestionnaire_signature.verifier_et_consommer(payload.token_signe)
        return {"valide": True, "departement": departement}
    except Exception as e:
