...
**Version:** 0.9 — Pre-submission draft
...
*Repository: github.com/taha-vera/Vera-protocole-*
...
# VERA Technical Pipeline
### Section 3 — Local Differential Privacy, Aggregation, and Audit Chain
...
or the substitution of one record (event-level adjacency). In tabular datasets,
...
Event-level adjacency is strictly stronger than event-level adjacency for radio
...
#### 6.1.1 Event-level Adjacency (see Section 4.4)

Le mécanisme ANCRE est prouvé sous le modèle de **event-level adjacency** (~_s) : les datasets D et D' ont la même taille n. Cette hypothèse est justifiée par H6 (fenêtre fermée avant agrégation), mais elle impose une contrainte opérationnelle forte.
...
The per-individual ε-DP guarantee relies on H1: one individual = one signal per session. A Sybil adversary submitting j signals via j distinct identities incurs a j×ε-DP privacy loss (Prop. 2.2, Dwork & Roth).
...
The discrete Laplace mechanism eliminates LSB leakage (Mironov, 2012). However, two side channels remain uncovered:

**Timing**: TMoM sorting runs in O(n log n) with data-dependent execution time. An adversary measuring response times may infer information about signal distribution. This channel is outside the standard honest-but-curious model but exploitable under an active adversary model.

**Error messages**: Errors (kill-switch, budget exhausted, replay) may reveal internal state. The server should return opaque errors in production.
...
2. **Preuve formelle sous event-level adjacency** — justifiée par la fermeture de fenêtre (H6), avec convention de tie-breaking explicite pour le Lemme 1.
